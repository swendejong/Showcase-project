...
{
    // A static dictionary to store active game sessions
    private static readonly Dictionary<string, GameSession> gameSessions = new();
    
   private static readonly List<string> _wordList = new()
{ 
...
[HttpGet("random")]
    public IActionResult GetRandomWord()
    {
        ...
        string word = _wordList[random.Next(_wordList.Count)];
        return Ok(new { word = word.ToUpper() });
    }
 ...
 public IActionResult CreateGame()
 {
     ...
     {
         GameCode = gameCode,
         Player1Id = gameCode + "0" // Assign Player 1 ID immediately
     };
     gameSessions[gameCode] = session;

     return Ok(new { gameCode, playerId = session.Player1Id });
 }
        ...
        public IActionResult JoinGame([FromBody] JoinGameRequest request)
        {
            ...
            {
                if (!session.SecondPlayerJoined)
                {
                    session.SecondPlayerJoined = true;
                    session.Player2Id = gameCode + "1"; // Assign player 2 identifier
                    return Ok(new { success = true, playerId = session.Player2Id });
                }
                return BadRequest(new { message = "Game is already full." });
            }

            return NotFound(new { message = "Game code not found." });
        }

        // Check if second player has joined
        [HttpGet("check-second-player/{gameCode}")]
        public IActionResult CheckSecondPlayer(string gameCode)
        {
            if (gameSessions.TryGetValue(gameCode, out var session))
            {
                return Ok(new { secondPlayerJoined = session.SecondPlayerJoined });
            }
        ...
        public IActionResult SubmitWord([FromBody] WordSubmission submission)
        {
            if (gameSessions.TryGetValue(submission.GameCode, out var session))
            {
                if (submission.PlayerId.EndsWith("0"))
                    session.Player1Word = submission.Word;
                if (submission.PlayerId.EndsWith("1"))
                    session.Player2Word = submission.Word;
        ...
        // Retrieve opponent's word
        [HttpGet("check-opponent-word/{gameCode}")]
        public IActionResult CheckOpponentWord(string gameCode, [FromQuery] string playerId)
        {
            if (gameSessions.TryGetValue(gameCode, out var session))
            {
                if (playerId.EndsWith("0") && session.Player2Word != null)
                    return Ok(new { word = session.Player2Word });

                if (playerId.EndsWith("1") && session.Player1Word != null)
                    return Ok(new { word = session.Player1Word });

                return Ok(new { word = (string?)null });
            }
    ...
    public class GameSession
    {
        ...
        public bool SecondPlayerJoined { get; set; } = false;
